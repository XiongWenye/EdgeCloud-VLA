#!/usr/bin/env python3
"""H200 gates for post-flow CloudEdge pi0.5 v3."""
...
def main() -> None:
    ...
    base.to(device).eval()
    cloudedge = CloudEdgePI05V3Policy.from_pretrained(args.bootstrap)
    cloudedge.to(device).eval()
    assert cloudedge.config.n_action_steps == 1
    assert cloudedge.config.history_window == 21
    ...
    noise = torch.randn(
        1,
        cloudedge.config.chunk_size,
        cloudedge.config.max_action_dim,
        device=device,
    )
    with torch.no_grad():
        base_chunk = base.model.sample_actions(
            base_images, base_masks, tokens, token_masks, noise=noise.clone()
        )
        cloud_context = cloudedge.model.encode_cloud_context(
            ce_images, ce_masks, tokens, token_masks
        )
        plan = cloudedge.model.sample_cloud_plan(cloud_context, noise=noise.clone())
        edge_context = cloudedge.model.encode_edge_images(ce_images, ce_masks)
        ce_chunk = cloudedge.model.correct_plan(plan, edge_context)
    cloud_sampler_parity = (base_chunk - plan.actions).abs().max().item()
    corrected_parity = (base_chunk - ce_chunk).abs().max().item()
    assert cloud_sampler_parity <= 1e-5, cloud_sampler_parity
    assert corrected_parity <= 1e-5, corrected_parity

    cloudedge.config.pretrained_path = args.bootstrap
    peft_policy = cloudedge.wrap_with_peft(
        peft_cli_overrides={"method_type": "LORA", "r": 8}
    )
    peft_policy.train()
    trainable = [
        name for name, parameter in peft_policy.named_parameters() if parameter.requires_grad
    ]
    unexpected = [
        name
    ...
    assert any(
        "paligemma.model.language_model" in name and "lora_" in name for name in trainable
    )
    ...
    # A cached cloud plan can be corrected again without invoking flow sampling.
    trained_policy = peft_policy.get_base_model()
    trained_model = trained_policy.model
    fake_plan = CloudPlan(
        actions=torch.randn(1, 10, 32, device=device),
        action_features=torch.randn(
            1, 10, trained_model.action_in_proj.out_features, device=device
        ),
    )
    edge_a = torch.randn(1, trained_model.edge_vision.config.hidden_size, device=device)
    edge_b = torch.randn(1, trained_model.edge_vision.config.hidden_size, device=device)
    with torch.no_grad():
        correction_a = trained_model.correct_plan(fake_plan, edge_a)
        correction_b = trained_model.correct_plan(fake_plan, edge_b)
    edge_reactivity = (correction_a - correction_b).abs().max().item()
    assert edge_reactivity > 0.0
    ...
    reloaded_base = CloudEdgePI05V3Policy.from_pretrained(
        args.bootstrap, config=saved_config
    )
    ...
    report = {
        ...
        "cloud_sampler_parity_max_abs": cloud_sampler_parity,
        "zero_correction_parity_max_abs": corrected_parity,
